...
$query = "SELECT video FROM meusite LIMIT 1";  // Alterado para buscar o vídeo atual
$resultado = mysqli_query($conexao, $query);
$dados = mysqli_fetch_assoc($resultado);
$video_anterior = $dados['video']; // Nome do vídeo anterior
...
if (isset($_FILES['novo_video']) && $_FILES['novo_video']['error'] === UPLOAD_ERR_OK) {
    ...
    if (@move_uploaded_file($novo_video_tmp, $destino)) {
        ...
        if (insereVideo($conexao, $novo_video)) {
